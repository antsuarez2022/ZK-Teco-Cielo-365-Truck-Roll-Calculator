...
st.set_page_config(
    page_title="cielo365 Partner ROI Calculator", 
    page_icon="⚡", 
    layout="centered"
)
...
st.markdown("""
    ...
    .logo-365 {
        color: #ff6a00 !important; /* ZKTeco/cielo365 Corporate Orange */
    }
    ...
    /* Streamlit Slider Widget Styling */
    div[data-testid="stSlider"] {
        padding-bottom: 10px;
    }
    
    /* Make widget labels bright and highly legible */
    ...
    /* Comprehensive Input Box Styling (Fixes White Box/Unreadable Text issue) */
    ...
    /* Browser Autofill & Focus state override */
...
# 4. Description
st.write(
    "Demonstrate the financial impact of cloud remote management. Security installers and dealers can calculate exactly how much "
    "they protect their contract margins by transitioning from expensive physical dispatches to cielo365."
)
...
st.markdown("### ⚙️ Installer Operations Profile")

with st.form(key="roi_form"):
    locations = st.slider(
        "How many client businesses / accounts is your company currently working on?", 
        min_value=1, 
        max_value=150, 
        value=20,
        help="The total number of client businesses, facilities, or physical access control accounts currently under active service contracts."
    )

    monthly_trips = st.slider(
        "Average physical service trips (truck rolls) per month, PER business?", 
        min_value=0, 
        max_value=10, 
        value=1,
        help="How many times a month a field technician drives out to a client business for minor adjustments or configurations."
    )
    ...
    with col2:
        st.markdown(f"""
            <div class="result-card savings-card">
                <div class="savings-header">🛡️ cielo365 Recovered Profit</div>
